    ...
    private void ejectOutdatedConnection() {
        try {
            ...
            for (Map.Entry<String, Connection> entry : connections.entrySet()) {
                ...
                // 当前时间-client上一次活跃时间=空闲时间，空闲时间超过KEEP_ALIVE_TIME，就是outDated
                // 空时时间太长，可能是Client挂了，也可能是太久没有相互发送数据了，所以后面会再检查一下Client到底挂没挂
            ...
            if (CollectionUtils.isNotEmpty(outDatedConnections)) {
                ...
                for (String outDateConnectionId : outDatedConnections) {
                    try {
                        ...
                        if (connection != null) {
                            // 尝试给Client发送ClientDetectionRequest请求
                ...
                // 真正outDate的连接，调用unregister()方法，将Server节点的服务实例信息清除掉
                for (String outDateConnectionId : outDatedConnections) {
