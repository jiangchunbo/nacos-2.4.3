...
public class ClientWorker implements Closeable {
    ...
    public void addTenantListeners(String dataId, String group, List<? extends Listener> listeners)
        ...
        // 先得到一个CacheData，默认会从本地磁盘获取dataId对应的配置数据
        // 一个GroupKey.getKeyTenant(dataId, group, tenant);对应一个CacheData
        CacheData cache = addCacheDataIfAbsent(dataId, group, tenant);
        synchronized (cache) {

            // 设置Listener给CacheData
            for (Listener listener : listeners) {
                cache.addListener(listener);
            }
            cache.setDiscard(false);
    ...
    public CacheData addCacheDataIfAbsent(String dataId, String group) {
        ...
        synchronized (cacheMap) {
            ...
            if (null != cacheFromMap) {
                ...
                // 一个taskId默认负责3000个配置的监听任务，最终一个taskId会对应一个RpcClient
    ...
    public class ConfigRpcTransportClient extends ConfigTransportClient {
        ...
        public void startInternal() {
            executor.schedule(() -> {
                while (!executor.isShutdown() && !executor.isTerminated()) {
                    try {
                        // addListener中会向listenExecutebell中添加元素，表示当前客户端添加了配置监听器
                        // 客户端监听的配置如果发生了改变，客户端会收到一个ConfigChangeNotifyRequest请求，并且往listenExecutebell添加元素
                        // listenExecutebell中如果有元素，就会立马执行监听器
                        // listenExecutebell中如果没有元素，就会等5秒，后面不一定会执行监听器
        ...
        public void executeConfigListen() throws NacosException {
            ...
            // addListener中会根据dataId、group生成对应的CacheData对象，表示要监听的配置
            for (CacheData cache : cacheMap.get().values()) {
                ...
                synchronized (cache) {
                    ...
                    // 如果CacheData的内容没有发生改变，会直接continue，如果改变了则会继续执行，从而执行Listener
                    ...
                    if (!cache.isDiscard()) {
                        // 按taskId进行分组，存在listenCachesMap中，一个taskId默认负责3000个配置的监听
        ...
        private boolean checkListenCache(Map<String, List<CacheData>> listenCachesMap) throws NacosException {
            ...
            if (!listenCachesMap.isEmpty()) {
                ...
                for (Map.Entry<String, List<CacheData>> entry : listenCachesMap.entrySet()) {
                    ...
                    // 为每个task构造一个RpcClient，每个RpcClient都会注册一个ServerRequestHandler
                    // 专门用来处理ConfigChangeNotifyRequest请求的，也就是服务端配置变更的通知
                    RpcClient rpcClient = ensureRpcClient(taskId);
                    ...
                    Future future = executorService.submit(() -> {
                        ...
                        // 把对哪些配置进行了监听，构造一个ConfigBatchListenRequest请求发送给服务端
                            ...
                            if (listenResponse != null && listenResponse.isSuccess()) {
                                ...
                                if (!CollectionUtils.isEmpty(changedConfigs)) {
                                    ...
                                    for (ConfigChangeBatchListenResponse.ConfigContext changeConfig : changedConfigs) {
                                        ...
                                        // 会运行一个NotifyTask（可异步执行，默认是同步，看Listener有没有配置Executor），从而去执行Listener
